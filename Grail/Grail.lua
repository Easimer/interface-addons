...
--				['R'] = 'Riding', ['S'] = 'Skinning', ['T'] = 'Tailoring', ['X'] = 'Archaeology', ['Z'] = 'First Aid', },
		...
		reputationExpansionMapping = {
			...
			[6] = { 1445, 1515, 1520, 1679, 1681, 1682, 1708, 1710, 1711, 1731, 1733, 1736, 1737, 1738, 1739, 1740, 1741, },
			},
...
--					self:AddQuestToMapArea(questId, self.quests[questId]['AZ'], self.mapAreaMapping[self.quests[questId]['AZ']])
			...
			mapIdsWithNames = nil
print("Total fixed milliseconds", self.totalFixedTime)	-- 614ms
print("Total locations milliseconds", totalLocationsTime)	-- 1191ms
...
--				local cachedStatus = self.questBits[questId] and self:_IntegerFromStringPosition(self.questBits[questId], 2) or nil
		...
		HasGarrisonBuilding = function(self, buildingId, ignoreIsBuildingRequirement)
			...
			if buildingId < 0 then
				desiredBildingTable = select(13, C_Garrison.GetBuildingInfo(-1 * buildingId))
			else
				desiredBildingTable = { buildingId }
			end
			for i = 1, #buildings do
				building = buildings[i]
				if tContains(desiredBildingTable, building.buildingID) then
		...
		HasGarrisonBuildingInPlot = function(self, buildingId, plotId)
			...
			if retval and (foundPlot == plotId) then
				retval = true
			end
			return retval, name, rank, foundPlot, foundBuildingId
		end,
...
print("_SendQuestChoiceResponse chooses:", anId, numericOption, "coords:", self:Coordinates())
